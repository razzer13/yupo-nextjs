...
export const metadata = {
  title: 'Dropshipping · YUPO Beauty',
  description: 'Vinde fără stoc propriu',
};
    ...
    <Providers>
      ...
      <main style={{ maxWidth:900, margin:'0 auto', padding:'24px 14px 60px' }}>
        <nav style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'var(--text3)', marginBottom:20 }}>
          ...
          <span>›</span><span style={{ color:'var(--text)', fontWeight:600 }}>Dropshipping</span>
        </nav>
        <div style={{ background:'linear-gradient(135deg,#1b5e20,#2e7d32)', borderRadius:14,
          padding:'40px 44px', marginBottom:24 }}>
          <h1 style={{ fontSize:28, fontWeight:800, color:'white', marginBottom:8 }}>Dropshipping</h1>
          <p style={{ fontSize:14, color:'rgba(255,255,255,.75)' }}>Vinde fără stoc propriu</p>
        </div>
        <div style={{ background:'white', borderRadius:12, padding:32, boxShadow:'var(--shadow)',
          ...
          <p>Pagina <strong>Dropshipping</strong> — conținut complet disponibil în curând.</p>
